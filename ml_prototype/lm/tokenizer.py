...
class SentencePieceTokenizer(Tokenizer):
    def __init__(self, config: Dict[str, Any]):
        ...
        self.model_path = os.path.join(self.token_folder_path, f"{self.model_name}.model")
        ...
        if os.path.exists(self.model_path):
            ...
            raise ValueError("Either provide a pretrained model path or an input file path for training.")
    ...
    def decode(self, tensor: torch.Tensor) -> str:
        if isinstance(tensor, torch.Tensor):
            tensor = tensor.cpu().tolist()
        return self.sp.decode_ids(tensor)